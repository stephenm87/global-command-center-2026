...
3. **Public Serper snapshot (`public-intel.js`, `refresh-public-intel.js`)**: Runs four fixed, balanced global-issues queries on the server, stores at most 20 normalized results in Netlify Blobs, and exposes the cached snapshot anonymously. The scheduled refresh runs every 30 minutes in production.
...
5. **Source health (`refresh-source-health.js`, `source-health.js`)**: Checks the generated editorial-source manifest daily and publishes non-authoritative availability states (`healthy`, `restricted`, `broken`, or `unknown`) from Netlify Blobs. A health response says whether the checker could reach a URL; it does not validate the article's claims.
